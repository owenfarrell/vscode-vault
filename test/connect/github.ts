...
xdescribe('Connect to Vault using GitHub Authentication', function() {
            ...
            context('when the Vaults section is expanded', function() {
                ...
                context('when the connect action is clicked', function() {
                    ...
                    context('when a valid URL is provided', function() {
                        before(async function() {
                            await input.setText(constants.CONNECT_GITHUB_VAULT_ADDRESS);
                            await input.confirm();
                        });
                        ...
                        context('when the friendly name is confirmed', function() {
                            ...
                            before(async function() {
                                await input.setText(constants.CONNECT_GITHUB_VAULT_NAME);
                                await input.confirm();
                            });
                            ...
                            it('provides the authentication type as a pickable option', async function() {
                                ...
                                quickPickItem = await input.findQuickPick('GitHub');
                            ...
                            context('when the authentication type is selected', function() {
                                ...
                                context('when the path is confirmed', function() {
                                    ...
                                    context('when the token is confirmed', function() {
                                        before(async function() {
                                            // TODO magic string
                                            await input.setText('GITHUB_TOKEN');
                                            await input.confirm();
                                        });
                                        ...
                                        it('creates a new server tree item', async function() {
                                            const viewItem = await section.findItem(constants.CONNECT_GITHUB_VAULT_NAME, 1);
