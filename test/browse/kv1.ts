            ...
            context('when the Vaults section is expanded', function() {
                ...
                context('when a server tree item exists', function() {
                    ...
                    it('has no children', async function() {
                        expect(await serverTreeItem.findChildItem(constants.BROWSE_KV1_MOUNT_POINT)).to.be.undefined;
                    });
                    ...
                    describe('add a valid path', function() {
                        ...
                        context('when a path is provided', function() {
                            before(async function() {
                                await input.setText(constants.BROWSE_KV1_FULL_PATH);
                                await input.confirm();
                            });
                            ...
                            it('the engine type is a pickable option', async function() {
                                ...
                                quickPickItem = await input.findQuickPick(constants.BROWSE_KV1_TYPE);
                                ...
                                context('when the server is expanded', function() {
                                    ...
                                    context('when the mount point is expanded', function() {
                                        ...
                                        it('includes the relative path as a child', async function() {
                                            expect(await mountPointTreeItem.findChildItem(constants.BROWSE_KV1_RELATIVE_PATH)).not.to.be.undefined;
                                        });
                    ...
                    describe('add a sibling path', function() {
                        ...
                        context('when a path is provided', function() {
                            before(async function() {
                                await input.setText(constants.BROWSE_KV1_FULL_SIBLING);
                                await input.confirm();
                            });
                            ...
                            it('includes the relative path as a child', async function() {
                                expect(await mountPointTreeItem.findChildItem(constants.BROWSE_KV1_RELATIVE_SIBLING)).not.to.be.undefined;
                            });
